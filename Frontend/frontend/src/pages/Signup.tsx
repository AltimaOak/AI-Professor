...
const getPasswordStrength = (pw: string) => {
  if (pw.length < 6) return { label: "Weak", color: "bg-destructive", width: "w-1/3" };
      ...
      <div className="flex min-h-screen pt-16">
        ...
        <div className="hidden lg:flex flex-1 items-center justify-center relative overflow-hidden">
          ...
          <blockquote className="relative z-10 max-w-md text-center px-8">
            ...
            <footer className="mt-4 text-sm text-muted-foreground">— Abigail Adams</footer>
          </blockquote>
        </div>
          ...
          <div className="w-full max-w-md bg-card rounded-2xl border border-primary/[0.15] p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input id="name" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 bg-bg-tertiary border-primary/20 text-foreground" placeholder="John Doe" />
                {errors.name && <p className="text-xs text-destructive mt-1">{errors.name}</p>}
              </div>

              <div>
                <Label htmlFor="email" className="text-foreground text-sm">Email</Label>
                <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 bg-bg-tertiary border-primary/20 text-foreground" placeholder="you@example.com" />
              ...
              <div>
                ...
                <div className="relative mt-1">
                  <Input
                    ...
                    className="bg-bg-tertiary border-primary/20 text-foreground pr-10"
                    placeholder="••••••••"
                  />
              ...
              <div>
                ...
                <Input id="confirm" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="mt-1 bg-bg-tertiary border-primary/20 text-foreground" placeholder="••••••••" />
                {errors.confirm && <p className="text-xs text-destructive mt-1">{errors.confirm}</p>}
              </div>

              <div className="flex items-start gap-2">
                <Checkbox id="terms" checked={agreed} onCheckedChange={(v) => setAgreed(v === true)} className="mt-0.5 border-primary/30 data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
              ...
              <Button type="submit" disabled={isLoading} className="w-full bg-primary text-primary-foreground hover:bg-accent-hover font-semibold rounded-xl h-11">
            ...
            <Button variant="outline" className="w-full border-primary/20 text-foreground hover:bg-secondary rounded-xl h-11" onClick={() => { /* TODO: Google OAuth */ }}>
            ...
            <p className="text-center text-sm text-muted-foreground mt-6">
              ...
              <Link to="/login" className="text-primary hover:text-accent-hover font-medium">Sign in</Link>
            </p>
          </div>
        </div>
      </div>
